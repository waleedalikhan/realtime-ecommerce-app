...
import { createProduct, updateProduct, setOrderStatus } from "../services/adminService.js";
import type { Server as SocketServer } from "socket.io";

function getIo(req: Request): SocketServer | undefined {
  return (req as Request & { app?: { get?: (k: string) => SocketServer } }).app?.get?.("io") as SocketServer | undefined;
}
...
export async function updateProductHandler(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    const product = await updateProduct(id, req.body as Parameters<typeof updateProduct>[1]);
    res.json(product);
  } catch (e) {
    if (e instanceof Error && "statusCode" in e) return next(e);
    next(e);
...
export async function updateOrderStatusHandler(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    const { id } = req.params as { id: string };
    const body = req.body as { status: "placed" | "paid" | "packed" | "shipped" | "delivered" };
